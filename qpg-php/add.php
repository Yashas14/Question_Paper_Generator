...
if(isset($_POST['submit'])){
        ...
        $marks = (int)$_REQUEST['Marks'];
        $CO = mysqli_real_escape_string($conn, $_REQUEST['Course_Outcome']);
        $RBT = mysqli_real_escape_string($conn, $_REQUEST['rbt']);
        $img=$_FILES['fileToUpload'];
        $i = mysqli_real_escape_string($conn, $img['name']);

        // Whitelist allowed subject table names to prevent SQL injection
        $allowed_subjects = ['UP', 'ADP', 'ATC', 'CNS', 'DBMS', 'ME'];
        if (!in_array($subject, $allowed_subjects)) {
            die('Invalid subject selected.');
        }

        $query = "INSERT INTO $subject(question,module,marks,co,rbt,img) values( '$question' , '$module' , '$marks' , '$CO' , '$RBT','$i')";
